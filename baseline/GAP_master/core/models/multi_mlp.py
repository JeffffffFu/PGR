...
from baseline.GAP_master.core.models import MLP
...
class MultiMLP(Module):
    ...
    def forward(self, x_stack: Tensor) -> Tensor:
        # 检查 x_stack 的维度，如果不是三维，则添加一个新的维度
       # print("x_stack2:",x_stack.shape)  #([2437, 16, 3])
        if x_stack.dim() == 2:
            x_stack = x_stack.unsqueeze(0)  # 添加一个新的维度，使其变为 (1, node, input_dim)
        elif x_stack.dim() != 3:
            raise ValueError(
                f"x_stack should be a 2-dimensional or 3-dimensional tensor, but got {x_stack.dim()}-dimensional tensor")
