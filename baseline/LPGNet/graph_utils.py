...
def get_comm_edges(from_ind, to_ind, adjacency, comms, dp, eps, rng,device):
    comm_edges = {}
    comm_edges_nondp = {}
    adj_ind = 0
    # 如果 adjacency 是 PyTorch 张量，则移动到 CPU 并转换为 NumPy 数组
    ...
    for i in range(from_ind, to_ind):
       ...
       # comm_edges[i] = torch.tensor(comm_edges[i], device=device)
        adj_ind += 1
    return comm_edges


def getCommunityCountsMP(
    adjacency, labels, num_classes, rng, device,dp=True, eps=2.0, num_proc=5
):
    comm_dict = {}
