...
from baseline.GAP_master.core.loggers import Logger
from torchmetrics import MeanMetric
from baseline.GAP_master.core.trainer.progress import TrainerProgress
from baseline.GAP_master.core.modules.base import Metrics, Stage, TrainableModule
...
class Trainer:
    ...
    def fit(self, 
        ...
        with self.progress:
            ...
            for epoch in range(1, epochs + 1):
                ...
                # train loop
                train_metrics,preds  = self.loop(train_dataloader, stage='train', prefix=prefix) #finding 4
                metrics.update(train_metrics)
                    
                # validation loop
                if val_dataloader and self.val_interval and epoch % self.val_interval == 0:

                    val_metrics,preds  = self.loop(val_dataloader, stage='val', prefix=prefix) #finding 4
                ...
                # test loop
                if test_dataloader:
                    test_metrics,preds  = self.loop(test_dataloader, stage='test', prefix=prefix) #finding 4
        ...
        return best_metrics,preds,self.model
    ...
    def loop(self, dataloader: Iterable, stage: Stage, prefix: str) -> Metrics:
        self.model.train(stage == 'train')    #如果stage==‘train’会train，否则
        self.progress.update(stage, visible=len(dataloader) > 1)

        for batch in dataloader:

            metrics,preds = self.step(batch, stage, prefix)   #finding 3
            for item in metrics:
        ...
        return self.aggregate_metrics(stage),preds
    ...
    def step(self, batch, stage: Stage, prefix: str) -> Metrics:
        ...
        loss, metrics,preds = self.model.step(batch, stage=stage)  #finding the train and test step 2
        ...
        return {f'{prefix}{stage}/{key}': value for key, value in metrics.items()},preds
