...
def main_func(dataset_name,eps=[0.5,1,1.5,2,2.5,3,3.5],e1_r=1/3,e2_r=1/3,N=20,t=1.0,exp_num=10,save_csv=False):
    ...
    if dataset_name=='Cora':
        ...
        data = dataset[0]
        mat0 = torch_geometric.utils.to_dense_adj(data.edge_index)[0]  # 从稀疏矩阵转成密集矩阵
        mat0= mat0.numpy().astype(np.uint8)   #按照privGraph的格式，需要转成np格式，注意这里没有自连接的
    else:
    ...
    for ei in range(len(eps)):
        ...
        for exper in range(exp_num):
            ...
            mat2 = mat2 + np.transpose(mat2)
            mat2[mat2>0] = 1
            print(f'mat2:{mat2}')   #这里得到了最后的邻接矩阵
            exit()
...
#     # set the dataset
#     # 'Facebook', 'CA-HepPh', 'Enron','Cora' ,'Chamelon'
