...
from baseline.GAP_master.core.modules.base import TrainableModule, Stage, Metrics
...
class MLPNodeClassifier(TrainableModule):
    ...
    def step(self, data: Data, stage: Stage) -> tuple[Optional[Tensor], Metrics]:
        ...
        else:    #finding the output preds 1
            print(f'preds:{len(preds)}')

        return loss, metrics,preds

    def logist(self, x) -> tuple[Optional[Tensor], Metrics]:
        preds = F.log_softmax(self(x), dim=-1)
       # logist=self(x)
        return preds
